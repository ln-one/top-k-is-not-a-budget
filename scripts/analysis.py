...
from replay import BUDGETS,KS,LS
...
def main(include_temporal=True):
 ...
 tr=[];cal=[];foldrows=[];ts=[]
 groups=[([ds],False) for ds in DATASETS]
 if include_temporal:groups += [([f'trec-covid-chrono-r{i}' for i in range(1,6)],True)]
