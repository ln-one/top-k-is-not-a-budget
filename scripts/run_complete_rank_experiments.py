...
from run_paper_experiments_v2 import BUDGETS,LS,preferred,ndcg,dumpcsv
from run_anytime_frontier_pilot import position_score,load_qrels,UnequalDepthCertificate
ROOT=Path(__file__).resolve().parents[1];ART=ROOT.parent/'artifacts/canonical-v1';OUT=ROOT/'results/paper-experiments-v3';RANKS=ROOT/'data/reconstructed-ranks-v1';KERNEL=ROOT/'results/completion-extension-v1/kernels.dylib'
...
def build(ds):
 ...
 old={}
 source=(ROOT/'data/inputs/experiments/target-validity/chronological-v1'/('round-'+ds.rsplit('r',1)[1])) if 'chrono' in ds else ROOT/'data/inputs/experiments/target-validity/static-v3'/ds
...
def replay(job):
 ...
 start=time.time();m=json.loads((p/f'{qid}.json').read_text());uu=np.load(p/'uuids.npy',mmap_mode='r');a=Stream(uu,np.load(p/f'{qid}-dense.npy',mmap_mode='r'));b=Stream(uu,np.load(p/f'{qid}-sparse.npy',mmap_mode='r'));full=m['full'];external=m['external'];jud=load_qrels(ROOT/'data/inputs/datasets'/ds/'source/qrels.tsv').get(qid,{})
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--dataset',choices=DS);ap.add_argument('--workers',type=int,default=2);ap.add_argument('--build-only',action='store_true');ap.add_argument('--static-only',action='store_true');args=ap.parse_args();OUT.mkdir(exist_ok=True);RANKS.mkdir(parents=True,exist_ok=True)
