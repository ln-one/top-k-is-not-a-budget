...
from rank_bounds import load_qrels
ROOT=Path(__file__).resolve().parents[1];OUT=Path(os.environ.get('DIBUD_OUTPUT', ROOT/'tmp/static'));RANKS=Path(os.environ.get('DIBUD_RANKS', ROOT/'data/reconstructed-ranks-v1'));INPUTS=Path(os.environ.get('DIBUD_INPUTS', ROOT/'data/inputs'))
def main():
 ...
 for a in audits:
  ds,qid=a['dataset'],a['query_id'];p=RANKS/ds;m=json.loads((p/f'{qid}.json').read_text());ids=m['external'];qrel=load_qrels(INPUTS/'datasets'/ds/'source/qrels.tsv').get(qid,{})
