...
from run_anytime_frontier_pilot import UnequalDepthCertificate,position_score,tie_key,load_qrels
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--output',type=Path,required=True);p.add_argument('--smoke',action='store_true');p.add_argument('--workers',type=int,default=4);args=p.parse_args()
    args.output.mkdir(parents=True,exist_ok=False);root=Path('data/inputs');jobs=[];sources=[]
    ...
    scripts=['run_paper_experiments_v2.py','pilot_grouped_frontier.py','run_anytime_frontier_pilot.py']
