...
我们比较同一读取轨迹的两种停止方式：完成精确 Top-20，或在预算耗尽时提前停止。通过五折校准，在校准查询上选择保留95%参考平均 nDCG@20 的最小预算，再用于留出查询。不足20条的位置按零增益计。

五个查询集保留了95.05%至97.68%的平均 nDCG@20，同时减少65.92%至99.53%的访问量（表3）。其中，TREC-DL 2019 的访问量减少99.53%，体现了提前停止对长尾完成成本的削减。
