...
给定查询、语料快照和读取预算 B，方法从 Dense 与 Sparse 排名中按需读取，返回已认证的完整 RRF 排序前缀。预算决定最多读取多少，认证决定能够返回多少。
...
从尚未输出的已见文档中选择下界最高者 x。若 ℓ(x) 超过其余文档的上界及 h_∅，便确认 x 为下一名，将其加入输出并继续检查。已见文档边界相等时按固定标识符顺序判定；对未见文档保持严格比较。每次输出都排除了所有剩余竞争者，因此累计结果始终与完整排序的对应前缀一致。

## 2.3 读取与停止

下一名无法确认时，选择剩余竞争者中上界最高的文档 y。若 h(y)≥h_∅，且 y 仅缺少一条通道的贡献，则优先推进该通道，以收紧其上界或取得实际名次。

否则，推进下一批能够使未读贡献上界下降更多的通道。对可读取 b 条且读取后仍开放的通道，下降量为 g(d_i+1)−g(d_i+b+1)。已耗尽通道不再读取，平分时按固定通道顺序选择。

每批读取后更新上下界，扩展已认证前缀。最后一批不超过剩余预算；预算耗尽或所有通道耗尽时，返回已有结果。
