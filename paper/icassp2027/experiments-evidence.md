...
- 固定L敏感性与迁移：transfer-depth-means.csv 由 transfer-grid.csv 按查询集、快照、深度取均值；DL19/20 保持语料与配置相同，时间曲线使用同30条查询及各轮标注。各曲线最佳深度仅为事后诊断，不能作为校准结果。冻结选择的留出检验使用：transfer-grid.csv、transfer-calibration.csv、transfer-test.csv、transfer-summary.csv。选择标准为线性nDCG10，校准和留出分离。
