...
Given a query, a corpus snapshot, and a read budget $B$, DiBud reads the dense and sparse rankings on demand and returns a certified prefix of their complete RRF ranking. The budget determines how much can be read; certification determines how much can be returned (Fig. [1](../preview/fig-method.svg)).

![DiBud overview. The budget directly limits reads, while current competition guides channel selection. Certification determines the returned prefix.](../preview/fig-method.svg)
...
$$ Here $B$ is the input limit; $K=|A|$ is the certified output size, not a preset count. Exactness applies to these returned positions. If none is certified, $A$ is empty. Reading the same document from both channels counts as two accesses.
...
The bounds ensure $\ell(x)\leq F(x)\leq h(x)$. Each emitted document therefore precedes every remaining document, including any not yet observed. Applying this argument at each output position establishes the prefix equality in [budget/output contract](04.method.md#Budget%20and%20output). Each position can be certified without resolving later positions, so the prefix grows incrementally and remains valid when the budget ends.
