...
x=np.arange(5);s=t.set_index('dataset').loc[DS];ax[0].bar(x,s.delta,color=colors,width=.6);ax[0].errorbar(x,s.delta,yerr=np.array([s.delta-s.ci_lower,s.ci_upper-s.delta]),fmt='none',color='#333333',capsize=2,lw=.8);ax[0].axhline(0,color='black',lw=.6);ax[0].set_xticks(x,N);ax[0].set_ylabel('nDCG@10 difference\nfrom full RRF');ax[0].set_title('(a) Held-out query transfer',loc='left')
s=t[t.dataset.str.contains('chrono')];x=np.arange(1,6);ax[1].plot(x,s.delta,'o-',color='#009988');ax[1].fill_between(x,s.ci_lower,s.ci_upper,color='#009988',alpha=.12);ax[1].axhline(0,color='black',lw=.6);ax[1].set_xticks(x);ax[1].set_xlabel('Corpus snapshot');ax[1].set_ylabel('nDCG@10 difference\nfrom full RRF');ax[1].set_title('(b) Frozen Round-1 selection',loc='left');save(fig,'transfer')
fig,ax=plt.subplots(2,1,figsize=(3.4,3.6),layout='constrained')
