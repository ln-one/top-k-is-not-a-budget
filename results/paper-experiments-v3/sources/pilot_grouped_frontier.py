...
from pilot_simple_schedulers import crossing
...
class GroupedFrontier:
    ...
    def event_amount(self,ch,limit):
        if self.candidate is None:return 1
        x=self.candidate;other=1-ch
        moving_mask=2 if ch==0 else 1
        fixed_mask=1 if ch==0 else 2
        moving=self.head(moving_mask,exclude=x[2])
        fixed=self.head(fixed_mask,self.unread(other),exclude=x[2])
        joint=self.head(3,exclude=x[2])
        base=max(self.unread(other),moving[0] if moving else -float('inf'))
        boundary=max(x[0],fixed[0] if fixed else -float('inf'),joint[0] if joint else -float('inf'))
        return crossing(self.depth[ch],boundary-base,limit)

def replay(a,b,exhausted,full,budgets,chunk,event=False):
    c=GroupedFrontier(a,b,exhausted);c.frontier();rows=[];elapsed=0.
    for B in budgets:
        t=time.perf_counter()
        while sum(c.depth)<min(B,sum(c.maximum)) and len(c.output)<100:
            amount=min(chunk,B-sum(c.depth));ch=c.choose(amount);amount=min(amount,c.maximum[ch]-c.depth[ch])
            if event:amount=c.event_amount(ch,amount)
            c.read(ch,amount);c.frontier()
...
def main():
    ...
    for idx,src in enumerate(m['sources'],1):
        ...
        for chunk,policy in ((1,'unit'),(64,'blocker64'),(64,'handoff64')):
            for r in replay(a,b,exhausted,d['full']['orderedPointIdsTop101'][:100],m['budgets'],chunk,event=policy=='handoff64'):
